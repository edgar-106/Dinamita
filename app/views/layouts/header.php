...
<head>
    ...
    .chooser-card { position: relative; min-height: clamp(330px, 48vh, 500px); overflow: hidden; display: flex; align-items: end; padding: 32px; border: 1px solid rgba(255,255,255,.15); background: center / cover; color: #fff; text-align: left; font: inherit; cursor: pointer; transition: transform .3s ease, border-color .3s ease; }
    .chooser-card::before { content: ""; position: absolute; inset: 0; background: linear-gradient(transparent 35%, rgba(0,0,0,.86)); }
    .chooser-card:hover, .chooser-card:focus-visible { transform: translateY(-6px); border-color: #c9a86a; outline: none; }
